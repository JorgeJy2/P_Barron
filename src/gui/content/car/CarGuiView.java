...
/**
 * Archivo: CarGuiView.java contiene la definición de la clase CarGuiView que
 * extiende de JPanel e implementa IObserver.
 * 
...
public class CarGuiView extends JPanel implements IObserver {
	 ...
	 * 
	 * //int random = (int) (10 * Math.random()); //String [] row = {
	 * "Modelo..."+random,"Placa...."+random,"Color..."+random };
	 * //DefaultTableModel model = (DefaultTableModel) table.getModel();
	 * //model.addRow(row);
	 * 
	 * Thread t = new Thread(() -> {
	 * 
	 * try { if(listCar.reloadNext()) {
	 * 
	 * reloadData();
	 * 
	 * }else { System.out.println("No se pudo cargar.."); }
	 * 
	 ...
	 * 
	 * while(inte.hasNext()) { DtoCar car =inte.next(); String[] carRow =
	 * {car.getModelo() + random, car.getPlaca() + random, car.getColor() + random};
	 * dm.addRow(carRow); }
	 * 
	 ...
	 * sp.getVerticalScrollBar().getMaximum()-100);
	 */
	/*
	 * }
	 */
	 ...
	 * 
	 * Interator<DtoCar> inte = listCar.getAll(); String[][] data= new
	 * String[listCar.sizeDtos()][3];
	 * 
	 * while(inte.hasNext()) { int countPeoples = inte.now(); DtoCar car
	 * =inte.next(); data[countPeoples][0] = car.getModelo(); data[countPeoples][1]
	 * = car.getPlaca(); data[countPeoples][2] = car.getColor(); } DefaultTableModel
	 * modelo = new DefaultTableModel(data,COLUMN_NAMES); table.setModel(modelo);
	 * table.setRowHeight(30); }
	 */
	/*
	 ...
	 * 
	 * while(inte.hasNext()) { DtoCar car =inte.next(); String[] carRow =
	 * {car.getModelo(), car.getPlaca(), car.getColor()}; tableModel.addRow(carRow);
	 * } }
	 */
